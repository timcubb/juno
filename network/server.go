...
func addUser(username string, channel chan Message) {
	...
	log.Printf("New Node %q", username)
...
func delUser(channel chan Message) {
	...
	log.Printf("Node disconnected %q", users.chans[channel])
...
func serve(id string, client io.ReadWriteCloser) {
	log.Printf("ComputeNode %q connected", id)
	defer log.Printf("ComputeNode %q disconnected", id)
	...
	user := <-login
	log.Printf("ComputeNode %q registered as %q", id, user.User)
